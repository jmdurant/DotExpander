...
function handlerSaveObject(type) {
    ...
    return async function () {
        return validationFunc(async (oldName, name, body, newParentfolder) => {
            console.log('[SAVE_HANDLER] Save handler called with:', { 
                oldName, 
                name, 
                body: body?.substring(0, 50) + '...', 
                newParentfolder: newParentfolder ? {
                    name: newParentfolder.name,
                    type: newParentfolder.type,
                    isFolderInstance: newParentfolder instanceof Folder,
                    hasListProperty: !!newParentfolder.list,
                    listLength: newParentfolder.list?.length
                } : undefined
            });
            
            // Only include folderPath if we're saving to a non-root folder
            const folderPath = newParentfolder ? [newParentfolder.name] : undefined;
            
            if (oldName) {
                // Send edit request to service worker
                console.log('[SAVE_HANDLER] Sending edit request:', {
                    snippetId: oldName,
                    name: name,
                    body: body?.substring(0, 50) + '...',
                ...
                chrome.runtime.sendMessage({
                    ...
                    console.log('[SAVE_HANDLER] Edit response:', response);
                    if (response.success) {
                        ...
                        console.error('[SAVE_HANDLER] Failed to edit snippet:', response.error);
                        alert(`Failed to edit snippet: ${response.error}`);
                    }
                });
            } else {
                // Send create request to service worker
                console.log('[SAVE_HANDLER] Sending create request:', {
                    name: name,
                    body: body?.substring(0, 50) + '...',
                    folderPath: folderPath
                });
                ...
                chrome.runtime.sendMessage({
                    ...
                    console.log('[SAVE_HANDLER] Create response:', response);
                    if (response.success) {
                        console.log('[SAVE_HANDLER] Snippet created successfully');
                        window.latestRevisionLabel = `created ${type} "${name}"`;
                        // Update UI after successful creation
                        saveSnippetData(undefined, newParentfolder ? newParentfolder.name : undefined);
                    } else {
                        console.error('[SAVE_HANDLER] Failed to create snippet:', response.error);
    ...
    return async function (callback) {
        ...
        console.log('Starting validation for:', { 
            ...
            selectedFolder: selectedFolder ? {
                ...
                isRoot: selectedFolder.name === Data.snippets.name
            } : null 
        });
        ...
        if (allValid) {
            ...
            // Only pass the folder if it's not the root "Snippets" folder
            const targetFolder = selectedFolder && selectedFolder.name !== Data.snippets.name ? selectedFolder : undefined;
            callback(oldName, name[0], body && body[0], targetFolder);
        }
    };
}
...
export function initSnippetWork() {
    ...
    chrome.storage.local.get('extensionUpdated', (result) => {
        isUpdate = result.extensionUpdated === true;
        
        if (isUpdate) {
            // Reset the flag
            chrome.storage.local.set({ 'extensionUpdated': false });
            
            // Show changelog using the correct class name
            qClsSingle("change-log").classList.add(SHOW_CLASS);
            qId("faded").classList.add(SHOW_CLASS);
        }
    });

    // Handle changelog close
    q(".change-log .buttons button").addEventListener("click", () => {
        chrome.storage.local.set({ 'extensionUpdated': false });
        qClsSingle("change-log").classList.remove(SHOW_CLASS);
        qId("faded").classList.remove(SHOW_CLASS);
