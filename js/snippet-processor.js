...
export function findSnippetMatch(textBeforeCursor) {
    ...
    const testText = match[0],
        snippetsList = Array.isArray(Data.snippets) ? Data.snippets.slice(2) : Data.snippets.list;
...
export function insertSnippetInInput(node, textBeforeCursor) {
    ...
    const cursorPosition = node.selectionStart,
        start = cursorPosition - matchingSnip.name.length,
        textBefore = node.value.substring(0, start),
        textAfter = node.value.substring(cursorPosition);
...
export function processSnippetInsertion(node, event) {
    ...
    if (node.isContentEditable) {
        result = insertSnippetInContentEditable(node, textBeforeCursor);
    } else {
        result = insertSnippetInInput(node, textBeforeCursor);
    }

    // Only reset states after successful insertion
    if (result) {
        // Reset all states including hotkey state
        window.resetModifierStates();
...
function resetHotkeyState() {
    // Clear any pending hotkey sequences
    if (window.resetModifierSequence) {
        window.resetModifierSequence();
    }

    // Reset any hotkey flags
    if (window.clearHotkeyFlags) {
        window.clearHotkeyFlags();
    }
}
