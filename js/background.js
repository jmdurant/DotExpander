...
// Add a promise to track Data initialization
const dataInitialized = new Promise((resolve) => {
    if (localStorage[LS_STORAGE_TYPE_PROP]) {
        ...
        storage.get(OLD_DATA_STORAGE_KEY, (response) => {
            window.Data = response[OLD_DATA_STORAGE_KEY] || SETTINGS_DEFAULTS;
            makeDataReady();
            resolve();
        });
    } else {
...
async function afterBGPageReload({
    ...
    chrome.notifications.create("", {
        type: "basic",
        iconUrl: "imgs/dotexpander-128.png",
        title: notifTitle,
        message: notifText,
    });
}
 ...
 */
async function onTabActivatedOrUpdated(tabId) {
    await dataInitialized;
    const tab = await chromeAPICallWrapper(chrome.tabs.get, tabId);
    if (!tab) {
        return;
    }
...
chrome.runtime.onInstalled.addListener((details) => {
    // Set extensionUpdated flag for changelog display
