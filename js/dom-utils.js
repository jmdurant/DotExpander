    ...
    if (node.tagName === "TEXTAREA" || node.tagName === "INPUT") {
        ...
        if (node.tagName === "TEXTAREA") {
            const text = node.value,
                lines = text.substr(0, node.selectionStart).split("\n");
...
function findTextNodeAtPosition(root, targetOffset) {
    ...
    const isRichText = root.isContentEditable,
        richTextOffset = isRichText ? 2 : 0,
        adjustedTargetOffset = Math.max(0, targetOffset - richTextOffset),
    ...
    for (let i = 0; i < textNodes.length; i++) {
        const nodeInfo = textNodes[i],
            ...
            nodeEnd = nodeStart + nodeInfo.length;
            ...
            if (nodeInfo.isPlaceholder) {
                ...
                if (placeholderMatch) {
                    const placeholderStart = nodeInfo.text.indexOf(placeholderMatch[0]),
                        placeholderEnd = placeholderStart + placeholderMatch[0].length;
    ...
    try {
        ...
        if (node.tagName === "TEXTAREA" || (node.tagName === "INPUT" && node.type === "text")) {
            ...
            const textLength = node.value.length,
                safeStart = Math.min(Math.max(0, start), textLength),
                safeEnd = Math.min(safeStart + length, textLength);
        ...
        const win = getNodeWindow(node),
            ...
            // Find start position
            startPos = findTextNodeAtPosition(node, start);
        if (!startPos) {
