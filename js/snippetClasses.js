                ...
                if (typeof content === "string" && content.trim().startsWith("{")) {
                    ...
                    if (parsed && parsed.html) {
                        if (parsed.type === "quill-table-content") {
                            return parsed.html
                                .replace(/<table/g, "<table class=\"dotexpander-table\"")
...
Snip.makeHTMLSuitableForTextarea = function (htmlNode) {
    ...
    try {
        // Check if we have a table content
        if (htmlNode.innerHTML.includes("quill-table-content")) {
            console.log("[DEBUG] Found table content");

            // Create a temporary div to parse the output
            const tempDiv = document.createElement("div");
            tempDiv.innerHTML = htmlNode.innerHTML;

            // Find the table in the output
            const table = tempDiv.querySelector("table");
            if (table) {
                console.log("[DEBUG] Found table element:", table.outerHTML);
                ...
                const tableData = rows.map((row, rowIndex) => {
                    console.log(`[DEBUG] Processing row ${rowIndex}:`, row.innerHTML);
                    return Array.from(row.querySelectorAll("td"))
                        .map((cell, cellIndex) => {
                            const p = cell.querySelector("p"),
                                content = (p ? p.textContent : cell.textContent).trim();
                            console.log(`[DEBUG] Cell [${rowIndex},${cellIndex}] content:`, content);
                            return content || " ";
                        });
                });
                ...
                for (let col = 0; col < tableData[0].length; col++) {
                    colWidths[col] = Math.max(3, ...tableData.map(row => (row[col] || "").length));
                }
                ...
                tableData.forEach((row) => {
                    asciiTable += `|${row.map((cell, colIndex) => ` ${cell.padEnd(colWidths[colIndex])} `).join("|")}|\n${separator}`;
                });

                console.log("[DEBUG] ASCII table:", asciiTable);
                return asciiTable;
            }

            // If no table found, try delta content
            const deltaMatch = htmlNode.innerHTML.match(/"delta":\s*({[^}]+})/);
            if (deltaMatch) {
                console.log("[DEBUG] Found delta content");
                try {
                    const deltaContent = JSON.parse(deltaMatch[1]);
                    if (deltaContent.ops) {
                        const textContent = deltaContent.ops
                            .map((op) => {
                                // Handle image ops
                                if (op.insert && typeof op.insert === "object" && op.insert.image) {
                                    return "[Pictures cannot be displayed in plain-text boxes.]";
                                }
                                return op.insert || "";
                            })
                            .join("")
                            .trim();
                        console.log("[DEBUG] Extracted text from delta:", textContent);
                        return textContent;
                    }
                } catch (e) {
                    console.log("[DEBUG] Error parsing delta:", e);
                }
            }
        }

        // If not table content or parsing failed, handle as regular HTML
        console.log("[DEBUG] Handling as regular HTML");
        ...
        console.log("[DEBUG] Final output:", finalString);
        return finalString;
    } catch (e) {
        console.log("[DEBUG] Error in makeHTMLSuitableForTextarea:", e);
...
function Folder(orgName, list, orgTimestamp, isSearchResultFolder) {
    console.log("[DEBUG] Creating new Folder:", {
        orgName, list, orgTimestamp, isSearchResultFolder,
    });
...
Folder.getDefaultSnippetData = function () {
    ...
    folder.list = [
        ...
        new Snip("urlMacro",
            ...
            ts),
        new Snip("README-New_UI_Details",
...
function DualTextbox($container, isTryItEditor) {
    ...
    this.setPlainText = function (text) {
        console.log("[DEBUG] Setting plain text input type:", typeof text);
        console.log("[DEBUG] Setting plain text input value:", text);
        ...
        try {
            ...
            // Handle object with body property
            if (text && typeof text === "object" && text.body) {
                console.log("[DEBUG] Found object with body property");
                console.log("[DEBUG] Body type:", typeof text.body);
                console.log("[DEBUG] Body value:", text.body);
                try {
                    content = JSON.parse(text.body);
                    console.log("[DEBUG] Successfully parsed body into:", content);
                } catch (e) {
                    console.log("[DEBUG] Error parsing body:", e);
                    console.log("[DEBUG] Failed to parse body:", text.body);
                }
            }
            // Handle string input
            else if (typeof text === "string") {
                console.log("[DEBUG] Attempting to parse string input");
                try {
                    content = JSON.parse(text);
                    console.log("[DEBUG] Successfully parsed string into:", content);
                } catch (e) {
                    console.log("[DEBUG] Failed to parse string:", text);
                }
            }
            // Use input as is
            else {
                console.log("[DEBUG] Using input as is:", text);
                content = text;
            }

            if (content && content.type === "quill-table-content") {
                console.log("[DEBUG] Found quill-table-content");
                console.log("[DEBUG] HTML content:", content.html);
                console.log("[DEBUG] Delta content:", content.delta);

                // For tables, first unescape the HTML
                const tempDiv = document.createElement("div");
                let unescapedHtml = content.html;

                // Log each transformation step
                console.log("[DEBUG] Step 1 - Original HTML:", unescapedHtml);
                ...
                unescapedHtml = unescapedHtml
                    ...
                    .replace(/width="[^"]*"/g, "")
                    .replace(/height="[^"]*"/g, "")
                    .replace(/<temporary[^>]*>.*?<\/temporary>/g, "")
                    .replace(/<br\/?>/g, "")
                    .replace(/<tbody>/g, "")
                    .replace(/<\/tbody>/g, "");

                console.log("[DEBUG] Step 2 - After cleanup:", unescapedHtml);

                tempDiv.innerHTML = unescapedHtml;
                console.log("[DEBUG] Step 3 - After setting innerHTML:", tempDiv.innerHTML);
                ...
                if (table) {
                    ...
                    const rows = Array.from(table.querySelectorAll("tr"));
                    console.log("[DEBUG] Found rows:", rows.length);
                    console.log("[DEBUG] Row elements:", rows.map(r => r.outerHTML));
                    ...
                    // Get cell contents for each row
                    const tableData = rows.map((row, rowIndex) => {
                        console.log(`[DEBUG] Processing row ${rowIndex} HTML:`, row.innerHTML);
                        const cells = Array.from(row.querySelectorAll("td"));
                        console.log(`[DEBUG] Found ${cells.length} cells in row ${rowIndex}`);
                        return cells.map((cell, cellIndex) => {
                            const p = cell.querySelector("p");
                            console.log(`[DEBUG] Cell [${rowIndex},${cellIndex}] HTML:`, cell.innerHTML);
                            console.log(`[DEBUG] Cell [${rowIndex},${cellIndex}] p element:`, p ? p.outerHTML : "none");
                            const content = (p ? p.textContent : cell.textContent).trim();
                            console.log(`[DEBUG] Cell [${rowIndex},${cellIndex}] extracted content:`, content);
                            return content || " "; // Use space for empty cells
                        });
                    });

                    console.log("[DEBUG] Final extracted table data:", tableData);

                    // Calculate column widths - find max width needed for each column
                    const colWidths = [];
                    for (let col = 0; col < tableData[0].length; col++) {
                        // Find the longest content in this column
                        colWidths[col] = Math.max(3, ...tableData.map(row => (row[col] || "").length));
                        console.log(`[DEBUG] Column ${col} width:`, colWidths[col]);
                    }

                    // Make all columns the same width by using the maximum width found
                    const maxWidth = Math.max(...colWidths);
                    colWidths.fill(maxWidth);
                    console.log("[DEBUG] Using consistent width for all columns:", maxWidth);

                    // Build ASCII table
                    let asciiTable = "\n";
                    const separator = `+${colWidths.map(w => "-".repeat(w + 2)).join("+")}+\n`;
                    console.log("[DEBUG] Separator line:", separator);

                    asciiTable += separator;
                    tableData.forEach((row, rowIndex) => {
                        const rowText = `|${row.map((cell, colIndex) => ` ${cell.padEnd(colWidths[colIndex])} `).join("|")}|\n`;
                        console.log(`[DEBUG] Row ${rowIndex} ASCII:`, rowText);
                        asciiTable += rowText + separator;
                    });

                    console.log("[DEBUG] Final ASCII table:", asciiTable);
                    this.$textarea.value = asciiTable;
                    return this;
                }
                console.log("[DEBUG] No table element found in cleaned HTML");
            } else {
                console.log("[DEBUG] Content is not quill-table-content:", content);
            }

            // Handle regular HTML content
            if (content && content.html) {
                console.log("[DEBUG] Handling as regular HTML content");
                const tempDiv = document.createElement("div");
            ...
            // Handle delta content only if table parsing failed
            if (content && content.delta && content.delta.ops) {
                console.log("[DEBUG] Handling as delta content");
                console.log("[DEBUG] Delta ops:", content.delta.ops);
                const textContent = content.delta.ops
                    .map(op => op.insert || "")
                    .join("")
                    .trim();
                console.log("[DEBUG] Extracted text from delta:", textContent);
            ...
            console.log("[DEBUG] Error in setPlainText:", e);
            console.log("[DEBUG] Error stack:", e.stack);
        }

        // Handle plain text
        console.log("[DEBUG] Falling back to plain text handling");
...
DualTextbox.prototype.loadContent = function (content) {
    ...
    try {
        ...
        const hasTableContent = html => html && (
            html.includes("<table")
                || html.includes("</table>")
                || html.includes("<td")
                || html.includes("</td>")
...
Snip.makeHTMLValidForExternalEmbed = function (html, isListingSnippets) {
    ...
    if (!normalizedHtml.includes("<p>")) {
        ...
        const paragraphs = normalizedHtml.split(/\n\s*\n/)
    ...
    tables.forEach((table, index) => {
        ...
        table.className = `${table.className.replace(/dotexpander-table/g, "")} dotexpander-table`;
    ...
    return new Promise((resolve) => {
        try {
            const waitForDependencies = (retryCount = 0) => {
                if (retryCount >= 50) {
                    console.error("[DEBUG] Dependencies not available after maximum retries");
                    resolve(null);
                    return;
                }

                if (typeof Quill === "undefined" || typeof QuillTableBetter === "undefined") {
                    console.log("[DEBUG] Waiting for Quill and QuillTableBetter...", retryCount);
                ...
                try {
                    ...
                    console.log("[DEBUG] Quill instance created");

                    resolve(quill);
                } catch (err) {
                    console.error("[DEBUG] Error during Quill initialization:", err);
                    resolve(null);
                }
            };
            ...
            console.error("[DEBUG] Error in initializeQuill:", err);
            resolve(null);
        }
    });
}
